...
def FMR_Lyapunov_map(alpha, gamma,B_ex,K,omega,phase, t,  t_eval, S0,sta_B,end_B,step_B,per,Lya_step,start_step,aspect = 12):
    ...
    for B in B_eval:
        ...
        B_list = np.append(B_list, [B])
    # print(B_list,poi_list)
    np.savetxt(f"csv/FMR_Lyapunovmap_Bz_{B_ex[2]}_Ky_{K[1]}_{omega}GHz._start_step_{start_step}_Lyastep_{Lya_step}_alpha{alpha}_paper_noswing_20-25.txt", np.stack([B_list,Lya_list]))
...
t = [0,4000]
t_eval = np.linspace(*t, 400000001)
#Lyapunov_map(1,32,176,8.092,t,t_eval,[0.4264,0,0], 2, 10, 400)
FMR_Lyapunov_map(0.02,0.176335977,[0,0,160],[0,0,0],28.4,[0,0,0],t,t_eval,[np.pi/2,0,0],20,25,51,[0,0.01,0], 1001,390000000,aspect = 3.5)
