...
def Lyapunov_map(alpha, beta, gamma,omega, t,  t_eval, X0,sta_B,end_B,step_B,per,Lya_step,start_step):
    B_ran = [sta_B, end_B]
    ...
    np.savetxt(f"csv/Duffing_Lyapunovmap_beta_{beta}_gamma_{gamma}_{omega}GHz._start_step_{start_step}_Lyastep_{Lya_step}_paper.txt",np.stack([B_list, Lya_list]))
    """
...
def FMR_Lyapunov_map(alpha, gamma,B_ex,K,omega,phase, t,  t_eval, S0,sta_B,end_B,step_B,per,Lya_step,start_step,aspect = 12):
    ...
    for B in B_eval:
        ...
        B_list = np.append(B_list, [B])
    # print(B_list,poi_list)
    np.savetxt(f"csv/maps_0.05/FMR_Lyapunovmap_Bx_{B_ex[0]}_Ky_{K[1]}_{omega}GHz._start_step_{start_step}_Lyastep_{Lya_step}_alpha{alpha}_paper_0-25.txt", np.stack([B_list,Lya_list]))
...
for Bx in B_x_list:
    if B_ani > Bx:
        ...
        Bx +=0.01
        omega = gamma * np.sqrt(Bx * (Bx - B_ani))
    FMR_Lyapunov_map(0.05, gamma, [Bx, 0, 0], [0, B_ani, 0], omega, [0, 0, 0], t, t_eval, [np.pi / 2, 0, 0], 0, 25,
