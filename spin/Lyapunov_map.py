...
def FMR_Lyapunov_map(alpha, gamma,B_ex,K,omega,phase, t,  t_eval, S0,sta_B,end_B,step_B,per,Lya_step,start_step,aspect = 12):
    ...
    for B in B_eval:
        ...
        B_list = np.append(B_list, [B])
    # print(B_list,poi_list)
    np.savetxt(f"csv/FMR_Lyapunovmap_Bx_{B_ex[0]}_Ky_{K[1]}_{omega}GHz._start_step_{start_step}_Lyastep_{Lya_step}_alpha{alpha}_paper.txt", np.stack([B_list,Lya_list]))
...
FMR_Lyapunov_map(0.02,0.176335977,[160,0,0],[0,0,0],21.36,[0,0,0],t,t_eval,[np.pi/2,0,0],0,500,251,[0.01,0,0], 1001,7000000,aspect = 3.5)
