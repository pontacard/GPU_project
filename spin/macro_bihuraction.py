...
def FMR_bihuraction(alpha, beta, gamma,ax,B,S0,omega, t,  t_eval,theta,Kx,Ky,Kz,start,stop,sta_B0,end_B0,step_B0):
    ...
    for B0 in B0_eval:
        ...
        duf.history()
        poi = duf.poincore(ax,78880000,80000001)
        B0_list = [B0] * len(poi)
    ...
    np.savetxt(f"csv/FMR_duffing_poincore_{B[0]}_{B[1]}_{B[2]}_{Kx}_{Ky}_{Kz}_{omega[1]}_alpha_{alpha}_paper_long.txt", np.stack([B_list,poi_list]))
...
t = [0, 8000]  # t(時間)が0〜100まで動き、その時のfを求める。
...
FMR_bihuraction(0.02, 0,0.176335977,1,[Bx,0,0],S0,[0,omega,0],t,t_eval,[0,0,0],0,0,0,0,900000,20,25,11)
